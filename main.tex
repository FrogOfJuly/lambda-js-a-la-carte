...
Sadly, direct use of the meta programming tools developed in the latter is prohibitively difficult due to outdated Metarocq\cite{sozeau2020metacoq}, but the overall structure of reasoning holds well even without them. 

The main idea is to separate closed inductive types into modular feature functors and non modular fixpoint. 
Resulting feature functors allow to per-functor proofs without relaying on a particular structure of overarching type.
Once desired proofs are complete its possible to assemble the type through inlined fixpoint application and proofs through open recursion technique.

The apporach is illustrated with an example \ref{fig:alacart_example} when the untyped lambda calculus is extended with booleans. 

There is more to it then that, so ones who 
