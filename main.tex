...
\section{Inlined functor fixed points}
...
The main idea is to separate closed inductive types into modular feature functors and non modular fixed point operation. 
Following this idea nievly will lead to incosistency, so the fixed point operation is not present explicitly, but inlined each time.
...
Once desired proofs are complete its possible to assemble the type through inlined fixed point application and proofs by "closing" the recursion over per-fucntor proofs.
